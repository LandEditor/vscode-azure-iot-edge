...
import type { BuildSettings } from "../src/common/buildSettings";
...
suite("utility tests", () => {
	...
	test("convertCreateOptions", async () => {
		...
		const oldOptionObj =
			deployment.modulesContent.$edgeAgent["properties.desired"].modules
				.SimulatedTemperatureSensor.settings.createOptions;
		...
		const settings =
			deployment.modulesContent.$edgeAgent["properties.desired"].modules
				.SimulatedTemperatureSensor.settings;
		...
		const optionString =
			...
			settings.createOptions01 +
			settings.createOptions02;
	...
	test("serializeCreateOptions", async () => {
		...
		const expected =
			'{"image":"test","createOptions":"{\\"Env\\":[\\"k1=v1\\",\\"k2=v2\\",\\"k3=v3\\"],' +
