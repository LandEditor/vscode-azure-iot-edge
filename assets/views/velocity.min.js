/*! VelocityJS.org (1.2.3). (C) 2014 Julian Shapiro. MIT @license: en.wikipedia.org/wiki/MIT_License */
/*! VelocityJS.org jQuery Shim (1.0.1). (C) 2014 The jQuery Foundation. MIT @license: en.wikipedia.org/wiki/MIT_License. */
!((a) => {
	function b(a) {
		const b = a.length;
		const d = c.type(a);
		return "function" === d || c.isWindow(a)
			? !1
			: 1 === a.nodeType && b
				? !0
				: "array" === d ||
					0 === b ||
					("number" === typeof b && b > 0 && b - 1 in a);
	}
	if (!a.jQuery) {
		const c = (a, b) => new c.fn.init(a, b);
		(c.isWindow = (a) => null != a && a === a.window),
			(c.type = (a) => null == a
					? `${a}`
					: "object" === typeof a || "function" === typeof a
						? e[g.call(a)] || "object"
						: typeof a),
			(c.isArray =
				Array.isArray ||
				(a) => "array" === c.type(a)),
			(c.isPlainObject = (a) => {
				let b;
				if (!a || "object" !== c.type(a) || a.nodeType || c.isWindow(a)) {
					return !1;
				}
				try {
					if (
						a.constructor &&
						!f.call(a, "constructor") &&
						!f.call(a.constructor.prototype, "isPrototypeOf")
					) {
						return !1;
					}
				} catch (d) {
					return !1;
				}
				for (b in a) {}
				return void 0 === b || f.call(a, b);
			}),
			(c.each = (a, c, d) => {
				let e;
				let f = 0;
				const g = a.length;
				const h = b(a);
				if (d) {
					if (h) {
						for (
							;
							g > f && ((e = c.apply(a[f], d)), e !== !1);
							f++
						) {}
					}
					else {
						for (f in a) {
							if (((e = c.apply(a[f], d)), e === !1)) { break; }
						}
					}
				} else if (h) {
					for (
						;
						g > f && ((e = c.call(a[f], f, a[f])), e !== !1);
						f++
					) {} }
				else {
					for (f in a) {
						if (((e = c.call(a[f], f, a[f])), e === !1)) { break; }
					}
				}
				return a;
			}),
			(c.data = (a, b, e) => {
				if (void 0 === e) {
					const f = a[c.expando];
					const g = f && d[f];
					if (void 0 === b) { return g; }
					if (g && b in g) { return g[b]; }
				} else if (void 0 !== b) {
					const f = a[c.expando] || (a[c.expando] = ++c.uuid);
					return (d[f] = d[f] || {}), (d[f][b] = e), e;
				}
			}),
			(c.removeData = (a, b) => {
				const e = a[c.expando];
				const f = e && d[e];
				f &&
					c.each(b, (a, b) => {
						delete f[b];
					});
			}),
			(c.extend = function () {
				let a;
				let b;
				let d;
				let e;
				let f;
				let g;
				let h = arguments[0] || {};
				let i = 1;
				const j = arguments.length;
				let k = !1;
				for (
					"boolean" === typeof h &&
						((k = h), (h = arguments[i] || {}), i++),
						"object" !== typeof h &&
							"function" !== c.type(h) &&
							(h = {}),
						i === j && ((h = this), i--);
					j > i;
					i++
				) {
					if (null != (f = arguments[i])) {
						for (e in f) {
							(a = h[e]),
								(d = f[e]),
								h !== d &&
									(k &&
									d &&
									(c.isPlainObject(d) || (b = c.isArray(d)))
										? (b
												? ((b = !1),
													(g =
														a && c.isArray(a)
															? a
															: []))
												: (g =
														a && c.isPlainObject(a)
															? a
															: {}),
											(h[e] = c.extend(k, g, d)))
										: void 0 !== d && (h[e] = d));
						}
					}
				}
				return h;
			}),
			(c.queue = (a, d, e) => {
				function f(a, c) {
					let d = c || [];
					return (
						null != a &&
							(b(Object(a))
								? !((a, b) => {
										for (
											let c = +b.length,
												d = 0,
												e = a.length;
											c > d;

										) {
											a[e++] = b[d++];
										}
										if (c !== c) {
											while (void 0 !== b[d]) {
												a[e++] = b[d++];
											}
										}
										return (a.length = e), a;
									})(d, "string" === typeof a ? [a] : a)
								: [].push.call(d, a)),
						d
					);
				}
				if (a) {
					d = `${(d || "fx")}queue`;
					let g = c.data(a, d);
					return e
						? (!g || c.isArray(e)
								? (g = c.data(a, d, f(e)))
								: g.push(e),
							g)
						: g || [];
				}
			}),
			(c.dequeue = (a, b) => {
				c.each(a.nodeType ? [a] : a, (a, d) => {
					b = b || "fx";
					const e = c.queue(d, b);
					let f = e.shift();
					"inprogress" === f && (f = e.shift()),
						f &&
							("fx" === b && e.unshift("inprogress"),
							f.call(d, () => {
								c.dequeue(d, b);
							}));
				});
			}),
			(c.fn = c.prototype =
				{
					init: function (a) {
						if (a.nodeType) { return (this[0] = a), this; }
						throw new Error("Not a DOM node.");
					},
					offset: function () {
						const b = this[0].getBoundingClientRect
							? this[0].getBoundingClientRect()
							: { top: 0, left: 0 };
						return {
							top:
								b.top +
								(a.pageYOffset || document.scrollTop || 0) -
								(document.clientTop || 0),
							left:
								b.left +
								(a.pageXOffset || document.scrollLeft || 0) -
								(document.clientLeft || 0),
						};
					},
					position: function () {
						function a() {
							for (
								let a = this.offsetParent || document;
								a &&
								"html" === !a.nodeType.toLowerCase &&
								"static" === a.style.position;

							) {
								a = a.offsetParent;
							}
							return a || document;
						}
						const b = this[0];
						const a = a.apply(b);
						const d = this.offset();
						const e = /^(?:body|html)$/i.test(a.nodeName)
								? { top: 0, left: 0 }
								: c(a).offset();
						return (
							(d.top -= parseFloat(b.style.marginTop) || 0),
							(d.left -= parseFloat(b.style.marginLeft) || 0),
							a.style &&
								((e.top +=
									parseFloat(a.style.borderTopWidth) || 0),
								(e.left +=
									parseFloat(a.style.borderLeftWidth) || 0)),
							{ top: d.top - e.top, left: d.left - e.left }
						);
					},
				});
		let d = {};
		(c.expando = `velocity${new Date().getTime()}`), (c.uuid = 0);
		for (
			let e = {},
				f = e.hasOwnProperty,
				g = e.toString,
				h =
					"Boolean Number String Function Array Date RegExp Object Error".split(
						" "
					),
				i = 0;
			i < h.length;
			i++
		) {
			e[`[object ${h[i]}]`] = h[i].toLowerCase();
		}
		(c.fn.init.prototype = c.fn), (a.Velocity = { Utilities: c });
	}
})(window),
	((a) => {
		"object" === typeof module && "object" === typeof module.exports
			? (module.exports = a())
			: "function" === typeof define && define.amd
				? define(a)
				: a();
	})(() => ((a, b, c, d) => {
			function e(a) {
				for (let b = -1, c = a ? a.length : 0, d = []; ++b < c; ) {
					const e = a[b];
					e && d.push(e);
				}
				return d;
			}
			function f(a) {
				return (
					p.isWrapped(a)
						? (a = [].slice.call(a))
						: p.isNode(a) && (a = [a]),
					a
				);
			}
			function g(a) {
				const b = m.data(a, "velocity");
				return null === b ? d : b;
			}
			function h(a) {
				return (b) => Math.round(b * a) * (1 / a);
			}
			function i(a, c, d, e) {
				function f(a, b) {
					return 1 - 3 * b + 3 * a;
				}
				function g(a, b) {
					return 3 * b - 6 * a;
				}
				function h(a) {
					return 3 * a;
				}
				function i(a, b, c) {
					return ((f(b, c) * a + g(b, c)) * a + h(b)) * a;
				}
				function j(a, b, c) {
					return 3 * f(b, c) * a * a + 2 * g(b, c) * a + h(b);
				}
				function k(b, c) {
					for (let e = 0; p > e; ++e) {
						const f = j(c, a, d);
						if (0 === f) { return c; }
						const g = i(c, a, d) - b;
						c -= g / f;
					}
					return c;
				}
				function l() {
					for (let b = 0; t > b; ++b) { x[b] = i(b * u, a, d); }
				}
				function m(b, c, e) {
					let f;
					let g;
					let h = 0;
					do {
						(g = c + (e - c) / 2),
							(f = i(g, a, d) - b),
							f > 0 ? (e = g) : (c = g);
					}
					while (Math.abs(f) > r && ++h < s);
					return g;
				}
				function n(b) {
					for (let c = 0, e = 1, f = t - 1; e !== f && x[e] <= b; ++e) {
						c += u;
					}
					--e;
					const g = (b - x[e]) / (x[e + 1] - x[e]);
					const h = c + g * u;
					const i = j(h, a, d);
					return i >= q ? k(b, h) : 0 === i ? h : m(b, c, c + u);
				}
				function o() {
					(y = !0), (a !== c || d !== e) && l();
				}
				const p = 4;
				const q = 0.001;
				const r = 1e-7;
				const s = 10;
				const t = 11;
				const u = 1 / (t - 1);
				const v = "Float32Array" in b;
				if (4 !== arguments.length) { return !1; }
				for (let w = 0; 4 > w; ++w) {
					if (
						"number" !== typeof arguments[w] ||
						Number.
						isNaN(arguments[w]) ||
						!Number.isFinite(arguments[w])
					) {
						return !1;
					}
				}
				(a = Math.min(a, 1)),
					(d = Math.min(d, 1)),
					(a = Math.max(a, 0)),
					(d = Math.max(d, 0));
				const x = v ? new Float32Array(t) : new Array(t);
				let y = !1;
				const z = (b) => (
							y || o(),
							a === c && d === e
								? b
								: 0 === b
									? 0
									: 1 === b
										? 1
										: i(n(b), c, e)
						);
				z.getControlPoints = () => [
						{ x: a, y: c },
						{ x: d, y: e },
					];
				const A = `generateBezier(${[a, c, d, e]})`;
				return (
					(z.toString = () => A),
					z
				);
			}
			function j(a, b) {
				let c = a;
				return (
					p.isString(a)
						? t.Easings[a] || (c = !1)
						: (c =
								p.isArray(a) && 1 === a.length
									? h.apply(null, a)
									: p.isArray(a) && 2 === a.length
										? u.apply(null, a.concat([b]))
										: p.isArray(a) && 4 === a.length
											? i.apply(null, a)
											: !1),
					c === !1 &&
						(c = t.Easings[t.defaults.easing]
							? t.defaults.easing
							: s),
					c
				);
			}
			function k(a) {
				if (a) {
					const b = new Date().getTime();
					const c = t.State.calls.length;
					c > 1e4 && (t.State.calls = e(t.State.calls));
					for (let f = 0; c > f; f++) {
						if (t.State.calls[f]) {
							const h = t.State.calls[f];
							const i = h[0];
							const j = h[2];
							let n = h[3];
							const o = !!n;
							let q = null;
							n || (n = t.State.calls[f][3] = b - 16);
							for (
								let r = Math.min((b - n) / j.duration, 1),
									s = 0,
									u = i.length;
								u > s;
								s++
							) {
								const w = i[s];
								const y = w.element;
								if (g(y)) {
									let z = !1;
									if (
										j.display !== d &&
										null !== j.display &&
										"none" !== j.display
									) {
										if ("flex" === j.display) {
											const A = [
												"-webkit-box",
												"-moz-box",
												"-ms-flexbox",
												"-webkit-flex",
											];
											m.each(A, (a, b) => {
												v.setPropertyValue(
													y,
													"display",
													b
												);
											});
										}
										v.setPropertyValue(
											y,
											"display",
											j.display
										);
									}
									j.visibility !== d &&
										"hidden" !== j.visibility &&
										v.setPropertyValue(
											y,
											"visibility",
											j.visibility
										);
									for (const B in w) {
										if ("element" !== B) {
											let C;
											const D = w[B];
											const E = p.isString(D.easing)
													? t.Easings[D.easing]
													: D.easing;
											if (1 === r) { C = D.endValue; }
											else {
												const F =
													D.endValue - D.startValue;
												if (
													((C =
														D.startValue +
														F * E(r, j, F)),
													!o && C === D.currentValue)
												) {
													continue;
												}
											}
											if (
												((D.currentValue = C),
												"tween" === B)
											) {
												q = C;
											}
											else {
												if (v.Hooks.registered[B]) {
													const G = v.Hooks.getRoot(B);
													const H =
															g(y)
																.rootPropertyValueCache[
																G
															];
													H &&
														(D.rootPropertyValue =
															H);
												}
												const I = v.setPropertyValue(
													y,
													B,
													D.currentValue +
														(0 === parseFloat(C)
															? ""
															: D.unitType),
													D.rootPropertyValue,
													D.scrollData
												);
												v.Hooks.registered[B] &&
													(g(
														y
													).rootPropertyValueCache[
														G
													] = v.Normalizations
														.registered[G]
														? v.Normalizations.registered[
																G
															](
																"extract",
																null,
																I[1]
															)
														: I[1]),
													"transform" === I[0] &&
														(z = !0);
											}
										}
									}
									j.mobileHA &&
										g(y).transformCache.translate3d === d &&
										((g(y).transformCache.translate3d =
											"(0px, 0px, 0px)"),
										(z = !0)),
										z && v.flushTransformCache(y);
								}
							}
							j.display !== d &&
								"none" !== j.display &&
								(t.State.calls[f][2].display = !1),
								j.visibility !== d &&
									"hidden" !== j.visibility &&
									(t.State.calls[f][2].visibility = !1),
								j.progress?.call(
										h[1],
										h[1],
										r,
										Math.max(0, n + j.duration - b),
										n,
										q
									),
								1 === r && l(f);
						}
					}
				}
				t.State.isTicking && x(k);
			}
			function l(a, b) {
				if (!t.State.calls[a]) { return !1; }
				for (
					let c = t.State.calls[a][0],
						e = t.State.calls[a][1],
						f = t.State.calls[a][2],
						h = t.State.calls[a][4],
						i = !1,
						j = 0,
						k = c.length;
					k > j;
					j++
				) {
					const l = c[j].element;
					if (
						(b ||
							f.loop ||
							("none" === f.display &&
								v.setPropertyValue(l, "display", f.display),
							"hidden" === f.visibility &&
								v.setPropertyValue(
									l,
									"visibility",
									f.visibility
								)),
						f.loop !== !0 &&
							(m.queue(l)[1] === d ||
								!/\.velocityQueueEntryFlag/i.test(
									m.queue(l)[1]
								)) &&
							g(l))
					) {
						(g(l).isAnimating = !1),
							(g(l).rootPropertyValueCache = {});
						let n = !1;
						m.each(v.Lists.transforms3D, (a, b) => {
							const c = /^scale/.test(b) ? 1 : 0;
							const e = g(l).transformCache[b];
							g(l).transformCache[b] !== d &&
								new RegExp(`^\\(${c}[^.]`).test(e) &&
								((n = !0), delete g(l).transformCache[b]);
						}),
							f.mobileHA &&
								((n = !0),
								g(l).transformCache.translate3d = undefined),
							n && v.flushTransformCache(l),
							v.Values.removeClass(l, "velocity-animating");
					}
					if (!b && f.complete && !f.loop && j === k - 1) {
						try {
							f.complete.call(e, e);
						} catch (o) {
							setTimeout(() => {
								throw o;
							}, 1);
						}
					}
					h && f.loop !== !0 && h(e),
						g(l) &&
							f.loop === !0 &&
							!b &&
							(m.each(g(l).tweensContainer, (a, b) => {
								/^rotate/.test(a) &&
									360 === parseFloat(b.endValue) &&
									((b.endValue = 0), (b.startValue = 360)),
									/^backgroundPosition/.test(a) &&
										100 === parseFloat(b.endValue) &&
										"%" === b.unitType &&
										((b.endValue = 0),
										(b.startValue = 100));
							}),
							t(l, "reverse", { loop: !0, delay: f.delay })),
						f.queue !== !1 && m.dequeue(l, f.queue);
				}
				t.State.calls[a] = !1;
				for (let p = 0, q = t.State.calls.length; q > p; p++) {
					if (t.State.calls[p] !== !1) {
						i = !0;
						break;
					}
				}
				i === !1 &&
					((t.State.isTicking = !1),
					t.State.calls = undefined,
					(t.State.calls = []));
			}
			let m
			const n = (() => {
					if (c.documentMode) { return c.documentMode; }
					for (let a = 7; a > 4; a--) {
						let b = c.createElement("div");
						if (
							((b.innerHTML =
								`<!--[if IE ${a}]><span></span><![endif]-->`),
							b.getElementsByTagName("span").length)
						) {
							return (b = null), a;
						}
					}
					return d;
				})()
			const o = (() => {
					let a = 0;
					return (
						b.webkitRequestAnimationFrame ||
						b.mozRequestAnimationFrame ||
						(b) => {
							let c;
							const d = new Date().getTime();
							return (
								(c = Math.max(0, 16 - (d - a))),
								(a = d + c),
								setTimeout(() => {
									b(d + c);
								}, c)
							);
						}
					);
				})()
			const p = {
					isString: (a) => "string" === typeof a,
					isArray:
						Array.isArray ||
						(a) => (
								"[object Array]" ===
								Object.prototype.toString.call(a)
							),
					isFunction: (a) => (
							"[object Function]" ===
							Object.prototype.toString.call(a)
						),
					isNode: (a) => a && a.nodeType,
					isNodeList: (a) => (
							"object" == typeof a &&
							/^\[object (HTMLCollection|NodeList|Object)\]$/.test(
								Object.prototype.toString.call(a)
							) &&
							a.length !== d &&
							(0 === a.length ||
								("object" == typeof a[0] && a[0].nodeType > 0))
						),
					isWrapped: (a) => (
							a && (a.jquery || (b.Zepto && b.Zepto.zepto.isZ(a)))
						),
					isSVG: (a) => b.SVGElement && a instanceof b.SVGElement,
					isEmptyObject: (a) => {
						for (const b in a) { return !1; }
						return !0;
					},
				},
				q = !1;
if (
	(a.fn?.jquery ? ((m = a), (q = !0)) : (m = b.Velocity.Utilities),
	8 >= n && !q)
) {
	throw new Error(
		"Velocity: IE8 and below require jQuery to be loaded before Velocity.",
	);
}
if (7 >= n) {
	return void (jQuery.fn.velocity = jQuery.fn.animate);
}
const r = 400;
const s = "swing";
let t = {
	State: {
		isMobile:
			/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(
				navigator.userAgent,
			),
		isAndroid: /Android/i.test(navigator.userAgent),
		isGingerbread: /Android 2\.3\.[3-7]/i.test(navigator.userAgent),
		isChrome: b.chrome,
		isFirefox: /Firefox/i.test(navigator.userAgent),
		prefixElement: c.createElement("div"),
		prefixMatches: {},
		scrollAnchor: null,
		scrollPropertyLeft: null,
		scrollPropertyTop: null,
		isTicking: !1,
		calls: [],
	},
	CSS: {},
	Utilities: m,
	Redirects: {},
	Easings: {},
	Promise: b.Promise,
	defaults: {
		queue: "",
		duration: r,
		easing: s,
		begin: d,
		complete: d,
		progress: d,
		display: d,
		visibility: d,
		loop: !1,
		delay: !1,
		mobileHA: !0,
		_cacheValues: !0,
	},
	init: (a) => {
		m.data(a, "velocity", {
			isSVG: p.isSVG(a),
			isAnimating: !1,
			computedStyle: null,
			tweensContainer: null,
			rootPropertyValueCache: {},
			transformCache: {},
		});
	},
	hook: null,
	mock: !1,
	version: { major: 1, minor: 2, patch: 2 },
	debug: !1,
};
b.pageYOffset !== d
	? ((t.State.scrollAnchor = b),
	  (t.State.scrollPropertyLeft = "pageXOffset"),
	  (t.State.scrollPropertyTop = "pageYOffset"))
	: ((t.State.scrollAnchor =
			c.documentElement || c.body.parentNode || c.body),
	  (t.State.scrollPropertyLeft = "scrollLeft"),
	  (t.State.scrollPropertyTop = "scrollTop"));
const u = (() => {
	function a(a) {
		return -a.tension * a.x - a.friction * a.v;
	}
	function b(b, c, d) {
		const e = {
			x: b.x + d.dx * c,
			v: b.v + d.dv * c,
			tension: b.tension,
			friction: b.friction,
		};
		return { dx: e.v, dv: a(e) };
	}
	function c(c, d) {
		const e = { dx: c.v, dv: a(c) };
		const f = b(c, 0.5 * d, e);
		const g = b(c, 0.5 * d, f);
		const h = b(c, d, g);
		const i = (1 / 6) * (e.dx + 2 * (f.dx + g.dx) + h.dx);
		const j = (1 / 6) * (e.dv + 2 * (f.dv + g.dv) + h.dv);
		return (c.x += i * d), (c.v += j * d), c;
	}
	return function d(a, b, e) {
		let f;
		let g;
		let h;
		const i = { x: -1, v: 0, tension: null, friction: null };
		const j = [0];
		let k = 0;
		const l = 1e-4;
		const m = 0.016;
		for (
			a = parseFloat(a) || 500,
				b = parseFloat(b) || 20,
				e = e || null,
				i.tension = a,
				i.friction = b,
				f = null !== e,
				f ? ((k = d(a, b)), (g = (k / e) * m)) : (g = m);
			;
		) {
			if (
				((h = c(h || i, g)),
				j.push(1 + h.x),
				(k += 16),
				!(Math.abs(h.x) > l && Math.abs(h.v) > l))
			) {
				break;
			}
		}
		return f ? (a) => j[(a * (j.length - 1)) | 0] : k;
	};
})();
(t.Easings = {
	linear: (a) => a,
	swing: (a) => 0.5 - Math.cos(a * Math.PI) / 2,
	spring: (a) => 1 - Math.cos(4.5 * a * Math.PI) * Math.exp(6 * -a),
}),
	m.each(
		[
			["ease", [0.25, 0.1, 0.25, 1]],
			["ease-in", [0.42, 0, 1, 1]],
			["ease-out", [0, 0, 0.58, 1]],
			["ease-in-out", [0.42, 0, 0.58, 1]],
			["easeInSine", [0.47, 0, 0.745, 0.715]],
			["easeOutSine", [0.39, 0.575, 0.565, 1]],
			["easeInOutSine", [0.445, 0.05, 0.55, 0.95]],
			["easeInQuad", [0.55, 0.085, 0.68, 0.53]],
			["easeOutQuad", [0.25, 0.46, 0.45, 0.94]],
			["easeInOutQuad", [0.455, 0.03, 0.515, 0.955]],
			["easeInCubic", [0.55, 0.055, 0.675, 0.19]],
			["easeOutCubic", [0.215, 0.61, 0.355, 1]],
			["easeInOutCubic", [0.645, 0.045, 0.355, 1]],
			["easeInQuart", [0.895, 0.03, 0.685, 0.22]],
			["easeOutQuart", [0.165, 0.84, 0.44, 1]],
			["easeInOutQuart", [0.77, 0, 0.175, 1]],
			["easeInQuint", [0.755, 0.05, 0.855, 0.06]],
			["easeOutQuint", [0.23, 1, 0.32, 1]],
			["easeInOutQuint", [0.86, 0, 0.07, 1]],
			["easeInExpo", [0.95, 0.05, 0.795, 0.035]],
			["easeOutExpo", [0.19, 1, 0.22, 1]],
			["easeInOutExpo", [1, 0, 0, 1]],
			["easeInCirc", [0.6, 0.04, 0.98, 0.335]],
			["easeOutCirc", [0.075, 0.82, 0.165, 1]],
			["easeInOutCirc", [0.785, 0.135, 0.15, 0.86]],
		],
		(a, b) => {
			t.Easings[b[0]] = i.apply(null, b[1]);
		},
	);
const v = (t.CSS = {
	RegEx: {
		isHex: /^#([A-f\d]{3}){1,2}$/i,
		valueUnwrap: /^[A-z]+\((.*)\)$/i,
		wrappedValueAlreadyExtracted: /[0-9.]+ [0-9.]+ [0-9.]+( [0-9.]+)?/,
		valueSplit: /([A-z]+\(.+\))|(([A-z0-9#-.]+?)(?=\s|$))/gi,
	},
	Lists: {
		colors: [
			"fill",
			"stroke",
			"stopColor",
			"color",
			"backgroundColor",
			"borderColor",
			"borderTopColor",
			"borderRightColor",
			"borderBottomColor",
			"borderLeftColor",
			"outlineColor",
		],
		transformsBase: [
			"translateX",
			"translateY",
			"scale",
			"scaleX",
			"scaleY",
			"skewX",
			"skewY",
			"rotateZ",
		],
		transforms3D: [
			"transformPerspective",
			"translateZ",
			"scaleZ",
			"rotateX",
			"rotateY",
		],
	},
	Hooks: {
		templates: {
			textShadow: ["Color X Y Blur", "black 0px 0px 0px"],
			boxShadow: ["Color X Y Blur Spread", "black 0px 0px 0px 0px"],
			clip: ["Top Right Bottom Left", "0px 0px 0px 0px"],
			backgroundPosition: ["X Y", "0% 0%"],
			transformOrigin: ["X Y Z", "50% 50% 0px"],
			perspectiveOrigin: ["X Y", "50% 50%"],
		},
		registered: {},
		register: () => {
			for (let a = 0; a < v.Lists.colors.length; a++) {
				const b =
					"color" === v.Lists.colors[a] ? "0 0 0 1" : "255 255 255 1";
				v.Hooks.templates[v.Lists.colors[a]] = [
					"Red Green Blue Alpha",
					b,
				];
			}
			let c;
			let d;
			let e;
			if (n) {
				for (c in v.Hooks.templates) {
					(d = v.Hooks.templates[c]), (e = d[0].split(" "));
					const f = d[1].match(v.RegEx.valueSplit);
					"Color" === e[0] &&
						(e.push(e.shift()),
						f.push(f.shift()),
						(v.Hooks.templates[c] = [e.join(" "), f.join(" ")]));
				}
			}
			for (c in v.Hooks.templates) {
				(d = v.Hooks.templates[c]), (e = d[0].split(" "));
				for (const a in e) {
					const g = c + e[a];
					const h = a;
					v.Hooks.registered[g] = [c, h];
				}
			}
		},
		getRoot: (a) => {
			const b = v.Hooks.registered[a];
			return b ? b[0] : a;
		},
		cleanRootPropertyValue: (a, b) => (
			v.RegEx.valueUnwrap.test(b) &&
				(b = b.match(v.RegEx.valueUnwrap)[1]),
			v.Values.isCSSNullValue(b) && (b = v.Hooks.templates[a][1]),
			b
		),
		extractValue: (a, b) => {
			const c = v.Hooks.registered[a];
			if (c) {
				const d = c[0];
				const e = c[1];
				return (
					(b = v.Hooks.cleanRootPropertyValue(d, b)),
					b.toString().match(v.RegEx.valueSplit)[e]
				);
			}
			return b;
		},
		injectValue: (a, b, c) => {
			const d = v.Hooks.registered[a];
			if (d) {
				let e;
				let f;
				const g = d[0];
				const h = d[1];
				return (
					(c = v.Hooks.cleanRootPropertyValue(g, c)),
					(e = c.toString().match(v.RegEx.valueSplit)),
					(e[h] = b),
					(f = e.join(" "))
				);
			}
			return c;
		},
	},
	Normalizations: {
		registered: {
			clip: (a, b, c) => {
				switch (a) {
					case "name":
						return "clip";
					case "extract": {
						let d;
						return (
							v.RegEx.wrappedValueAlreadyExtracted.test(c)
								? (d = c)
								: ((d = c
										.toString()
										.match(v.RegEx.valueUnwrap)),
								  (d = d ? d[1].replace(/,(\s+)?/g, " ") : c)),
							d
						);
					}
					case "inject":
						return `rect(${c})`;
				}
			},
			blur: (a, b, c) => {
				switch (a) {
					case "name":
						return t.State.isFirefox ? "filter" : "-webkit-filter";
					case "extract": {
						let d = parseFloat(c);
						if (!d && 0 !== d) {
							const e = c
								.toString()
								.match(/blur\(([0-9]+[A-z]+)\)/i);
							d = e ? e[1] : 0;
						}
						return d;
					}
					case "inject":
						return parseFloat(c) ? `blur(${c})` : "none";
				}
			},
			opacity: (a, b, c) => {
				if (8 >= n) {
					switch (a) {
						case "name":
							return "filter";
						case "extract": {
							const d = c
								.toString()
								.match(/alpha\(opacity=(.*)\)/i);
							return (c = d ? d[1] / 100 : 1);
						}
						case "inject":
							return (
								(b.style.zoom = 1),
								parseFloat(c) >= 1
									? ""
									: `alpha(opacity=${parseInt(
											100 * parseFloat(c),
											10,
									  )})`
							);
					}
				} else {
					switch (a) {
						case "name":
							return "opacity";
						case "extract":
							return c;
						case "inject":
							return c;
					}
				}
			},
		},
		register: () => {
			9 >= n ||
				t.State.isGingerbread ||
				(v.Lists.transformsBase = v.Lists.transformsBase.concat(
					v.Lists.transforms3D,
				));
			for (let a = 0; a < v.Lists.transformsBase.length; a++) {
				!(() => {
					const b = v.Lists.transformsBase[a];
					v.Normalizations.registered[b] = (a, c, e) => {
						switch (a) {
							case "name":
								return "transform";
							case "extract":
								return g(c) === d ||
									g(c).transformCache[b] === d
									? /^scale/i.test(b)
										? 1
										: 0
									: g(c).transformCache[b].replace(
											/[()]/g,
											"",
									  );
							case "inject": {
								let f = !1;
								switch (b.substr(0, b.length - 1)) {
									case "translate": {
										f = !/(%|px|em|rem|vw|vh|\d)$/i.test(e);
										break;
									}
									case "scal":
									case "scale": {
										t.State.isAndroid &&
											g(c).transformCache[b] === d &&
											1 > e &&
											(e = 1),
											(f = !/(\d)$/i.test(e));
										break;
									}
									case "skew": {
										f = !/(deg|\d)$/i.test(e);
										break;
									}
									case "rotate":
										f = !/(deg|\d)$/i.test(e);
								}
								return (
									f || (g(c).transformCache[b] = `(${e})`),
									g(c).transformCache[b]
								);
							}
						}
					};
				})();
			}
			for (let a = 0; a < v.Lists.colors.length; a++) {
				!(() => {
					const b = v.Lists.colors[a];
					v.Normalizations.registered[b] = (a, c, e) => {
						switch (a) {
							case "name":
								return b;
							case "extract": {
								let f;
								if (
									v.RegEx.wrappedValueAlreadyExtracted.test(e)
								) {
									f = e;
								} else {
									let g;
									const h = {
										black: "rgb(0, 0, 0)",
										blue: "rgb(0, 0, 255)",
										gray: "rgb(128, 128, 128)",
										green: "rgb(0, 128, 0)",
										red: "rgb(255, 0, 0)",
										white: "rgb(255, 255, 255)",
									};
									/^[A-z]+$/i.test(e)
										? (g = h[e] !== d ? h[e] : h.black)
										: v.RegEx.isHex.test(e)
										  ? (g = `rgb(${v.Values.hexToRgb(
													e,
											  ).join(" ")})`)
										  : /^rgba?\(/i.test(e) ||
											  (g = h.black),
										(f = (g || e)
											.toString()
											.match(v.RegEx.valueUnwrap)[1]
											.replace(/,(\s+)?/g, " "));
								}
								return (
									8 >= n ||
										3 !== f.split(" ").length ||
										(f += " 1"),
									f
								);
							}
							case "inject":
								return (
									8 >= n
										? 4 === e.split(" ").length &&
										  (e = e
												.split(/\s+/)
												.slice(0, 3)
												.join(" "))
										: 3 === e.split(" ").length &&
										  (e += " 1"),
									`${8 >= n ? "rgb" : "rgba"}(${e
										.replace(/\s+/g, ",")
										.replace(/\.(\d)+(?=,)/g, "")})`
								);
						}
					};
				})();
			}
		},
	},
	Names: {
		camelCase: (a) => a.replace(/-(\w)/g, (a, b) => b.toUpperCase()),
		SVGAttribute: (a) => {
			let b = "width|height|x|y|cx|cy|r|rx|ry|x1|x2|y1|y2";
			return (
				(n || (t.State.isAndroid && !t.State.isChrome)) &&
					(b += "|transform"),
				new RegExp(`^(${b})$`, "i").test(a)
			);
		},
		prefixCheck: (a) => {
			if (t.State.prefixMatches[a]) {
				return [t.State.prefixMatches[a], !0];
			}
			for (
				let b = ["", "Webkit", "Moz", "ms", "O"], c = 0, d = b.length;
				d > c;
				c++
			) {
				let e;
				if (
					((e =
						0 === c
							? a
							: b[c] + a.replace(/^\w/, (a) => a.toUpperCase())),
					p.isString(t.State.prefixElement.style[e]))
				) {
					return (t.State.prefixMatches[a] = e), [e, !0];
				}
			}
			return [a, !1];
		},
	},
	Values: {
		hexToRgb: (a) => {
			let b;
			const c = /^#?([a-f\d])([a-f\d])([a-f\d])$/i;
			const d = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i;
			return (
				(a = a.replace(c, (a, b, c, d) => b + b + c + c + d + d)),
				(b = d.exec(a)),
				b
					? [
							parseInt(b[1], 16),
							parseInt(b[2], 16),
							parseInt(b[3], 16),
					  ]
					: [0, 0, 0]
			);
		},
		isCSSNullValue: (a) =>
			0 === a ||
			/^(none|auto|transparent|(rgba\(0, ?0, ?0, ?0\)))$/i.test(a),
		getUnitType: (a) =>
			/^(rotate|skew)/i.test(a)
				? "deg"
				: /(^(scale|scaleX|scaleY|scaleZ|alpha|flexGrow|flexHeight|zIndex|fontWeight)$)|((opacity|red|green|blue|alpha)$)/i.test(
							a,
					  )
				  ? ""
				  : "px",
		getDisplayType: (a) => {
			const b = a?.tagName.toString().toLowerCase();
			return /^(b|big|i|small|tt|abbr|acronym|cite|code|dfn|em|kbd|strong|samp|var|a|bdo|br|img|map|object|q|script|span|sub|sup|button|input|label|select|textarea)$/i.test(
				b,
			)
				? "inline"
				: /^(li)$/i.test(b)
				  ? "list-item"
				  : /^(tr)$/i.test(b)
					  ? "table-row"
					  : /^(table)$/i.test(b)
						  ? "table"
						  : /^(tbody)$/i.test(b)
							  ? "table-row-group"
							  : "block";
		},
		addClass: (a, b) => {
			a.classList
				? a.classList.add(b)
				: (a.className += (a.className.length ? " " : "") + b);
		},
		removeClass: (a, b) => {
			a.classList
				? a.classList.remove(b)
				: (a.className = a.className
						.toString()
						.replace(
							new RegExp(
								`(^|\\s)${b.split(" ").join("|")}(\\s|$)`,
								"gi",
							),
							" ",
						));
		},
	},
	getPropertyValue: (a, c, e, f) => {
		function h(a, c) {
			function e() {
				j && v.setPropertyValue(a, "display", "none");
			}
			let i = 0;
			if (8 >= n) {
				i = m.css(a, c);
			} else {
				let j = !1;
				if (
					(/^(width|height)$/.test(c) &&
						0 === v.getPropertyValue(a, "display") &&
						((j = !0),
						v.setPropertyValue(
							a,
							"display",
							v.Values.getDisplayType(a),
						)),
					!f)
				) {
					if (
						"height" === c &&
						"border-box" !==
							v
								.getPropertyValue(a, "boxSizing")
								.toString()
								.toLowerCase()
					) {
						const k =
							a.offsetHeight -
							(parseFloat(
								v.getPropertyValue(a, "borderTopWidth"),
							) || 0) -
							(parseFloat(
								v.getPropertyValue(a, "borderBottomWidth"),
							) || 0) -
							(parseFloat(v.getPropertyValue(a, "paddingTop")) ||
								0) -
							(parseFloat(
								v.getPropertyValue(a, "paddingBottom"),
							) || 0);
						return e(), k;
					}
					if (
						"width" === c &&
						"border-box" !==
							v
								.getPropertyValue(a, "boxSizing")
								.toString()
								.toLowerCase()
					) {
						const l =
							a.offsetWidth -
							(parseFloat(
								v.getPropertyValue(a, "borderLeftWidth"),
							) || 0) -
							(parseFloat(
								v.getPropertyValue(a, "borderRightWidth"),
							) || 0) -
							(parseFloat(v.getPropertyValue(a, "paddingLeft")) ||
								0) -
							(parseFloat(
								v.getPropertyValue(a, "paddingRight"),
							) || 0);
						return e(), l;
					}
				}
				let o;
				(o =
					g(a) === d
						? b.getComputedStyle(a, null)
						: g(a).computedStyle
						  ? g(a).computedStyle
						  : (g(a).computedStyle = b.getComputedStyle(a, null))),
					"borderColor" === c && (c = "borderTopColor"),
					(i =
						9 === n && "filter" === c
							? o.getPropertyValue(c)
							: o[c]),
					("" === i || null === i) && (i = a.style[c]),
					e();
			}
			if ("auto" === i && /^(top|right|bottom|left)$/i.test(c)) {
				const p = h(a, "position");
				("fixed" === p || ("absolute" === p && /top|left/i.test(c))) &&
					(i = `${m(a).position()[c]}px`);
			}
			return i;
		}
		let i;
		if (v.Hooks.registered[c]) {
			const j = c;
			const k = v.Hooks.getRoot(j);
			e === d && (e = v.getPropertyValue(a, v.Names.prefixCheck(k)[0])),
				v.Normalizations.registered[k] &&
					(e = v.Normalizations.registered[k]("extract", a, e)),
				(i = v.Hooks.extractValue(j, e));
		} else if (v.Normalizations.registered[c]) {
			let l;
			let o;
			(l = v.Normalizations.registered[c]("name", a)),
				"transform" !== l &&
					((o = h(a, v.Names.prefixCheck(l)[0])),
					v.Values.isCSSNullValue(o) &&
						v.Hooks.templates[c] &&
						(o = v.Hooks.templates[c][1])),
				(i = v.Normalizations.registered[c]("extract", a, o));
		}
		if (!/^[\d-]/.test(i)) {
			if (g(a)?.isSVG && v.Names.SVGAttribute(c)) {
				if (/^(height|width)$/i.test(c)) {
					try {
						i = a.getBBox()[c];
					} catch (p) {
						i = 0;
					}
				} else {
					i = a.getAttribute(c);
				}
			} else {
				i = h(a, v.Names.prefixCheck(c)[0]);
			}
		}
		return (
			v.Values.isCSSNullValue(i) && (i = 0),
			t.debug >= 2 && console.log(`Get ${c}: ${i}`),
			i
		);
	},
	setPropertyValue: (a, c, d, e, f) => {
		let h = c;
		if ("scroll" === c) {
			f.container
				? (f.container[`scroll${f.direction}`] = d)
				: "Left" === f.direction
				  ? b.scrollTo(d, f.alternateValue)
				  : b.scrollTo(f.alternateValue, d);
		} else if (
			v.Normalizations.registered[c] &&
			"transform" === v.Normalizations.registered[c]("name", a)
		) {
			v.Normalizations.registered[c]("inject", a, d),
				(h = "transform"),
				(d = g(a).transformCache[c]);
		} else {
			if (v.Hooks.registered[c]) {
				const i = c;
				const j = v.Hooks.getRoot(c);
				(e = e || v.getPropertyValue(a, j)),
					(d = v.Hooks.injectValue(i, d, e)),
					(c = j);
			}
			if (
				(v.Normalizations.registered[c] &&
					((d = v.Normalizations.registered[c]("inject", a, d)),
					(c = v.Normalizations.registered[c]("name", a))),
				(h = v.Names.prefixCheck(c)[0]),
				8 >= n)
			) {
				try {
					a.style[h] = d;
				} catch (k) {
					t.debug &&
						console.log(
							`Browser does not support [${d}] for [${h}]`,
						);
				}
			} else {
				g(a)?.isSVG && v.Names.SVGAttribute(c)
					? a.setAttribute(c, d)
					: (a.style[h] = d);
			}
			t.debug >= 2 && console.log(`Set ${c} (${h}): ${d}`);
		}
		return [h, d];
	},
	flushTransformCache: (a) => {
		function b(b) {
			return parseFloat(v.getPropertyValue(a, b));
		}
		let c = "";
		if ((n || (t.State.isAndroid && !t.State.isChrome)) && g(a).isSVG) {
			const d = {
				translate: [b("translateX"), b("translateY")],
				skewX: [b("skewX")],
				skewY: [b("skewY")],
				scale:
					1 !== b("scale")
						? [b("scale"), b("scale")]
						: [b("scaleX"), b("scaleY")],
				rotate: [b("rotateZ"), 0, 0],
			};
			m.each(g(a).transformCache, (a) => {
				/^translate/i.test(a)
					? (a = "translate")
					: /^scale/i.test(a)
					  ? (a = "scale")
					  : /^rotate/i.test(a) && (a = "rotate"),
					d[a] && ((c += `${a}(${d[a].join(" ")}) `), delete d[a]);
			});
		} else {
			let e;
			let f;
			m.each(
				g(a).transformCache,
				(b) => (
					(e = g(a).transformCache[b]),
					"transformPerspective" === b
						? ((f = e), !0)
						: (9 === n && "rotateZ" === b && (b = "rotate"),
						  void (c += `${b + e} `))
				),
			),
				f && (c = `perspective${f} ${c}`);
		}
		v.setPropertyValue(a, "transform", c);
	},
});
v.Hooks.register(),
	v.Normalizations.register(),
	(t.hook = (a, b, c) => {
		let e = d;
		return (
			(a = f(a)),
			m.each(a, (a, f) => {
				if ((g(f) === d && t.init(f), c === d)) {
					e === d && (e = t.CSS.getPropertyValue(f, b));
				} else {
					const h = t.CSS.setPropertyValue(f, b, c);
					"transform" === h[0] && t.CSS.flushTransformCache(f),
						(e = h);
				}
			}),
			e
		);
	});
const w = function () {
	function a() {
		return h ? B.promise || null : i;
	}
	function e() {
		function a() {
			function a(a, b) {
				let c = d;
				let e = d;
				let g = d;
				return (
					p.isArray(a)
						? ((c = a[0]),
						  (!p.isArray(a[1]) && /^[\d-]/.test(a[1])) ||
						  p.isFunction(a[1]) ||
						  v.RegEx.isHex.test(a[1])
								? (g = a[1])
								: ((p.isString(a[1]) &&
										!v.RegEx.isHex.test(a[1])) ||
										p.isArray(a[1])) &&
								  ((e = b ? a[1] : j(a[1], h.duration)),
								  a[2] !== d && (g = a[2])))
						: (c = a),
					b || (e = e || h.easing),
					p.isFunction(c) && (c = c.call(f, y, x)),
					p.isFunction(g) && (g = g.call(f, y, x)),
					[c || 0, e, g]
				);
			}
			function l(a, b) {
				let c;
				let d;
				return (
					(d = (b || "0")
						.toString()
						.toLowerCase()
						.replace(/[%A-z]+$/, (a) => (c = a), "")),
					c || (c = v.Values.getUnitType(a)),
					[d, c]
				);
			}
			function n() {
				const a = {
					myParent: f.parentNode || c.body,
					position: v.getPropertyValue(f, "position"),
					fontSize: v.getPropertyValue(f, "fontSize"),
				};
				const d =
					a.position === I.lastPosition &&
					a.myParent === I.lastParent;
				const e = a.fontSize === I.lastFontSize;
				(I.lastParent = a.myParent),
					(I.lastPosition = a.position),
					(I.lastFontSize = a.fontSize);
				const h = 100;
				const i = {};
				if (e && d) {
					(i.emToPx = I.lastEmToPx),
						(i.percentToPxWidth = I.lastPercentToPxWidth),
						(i.percentToPxHeight = I.lastPercentToPxHeight);
				} else {
					const j = g(f).isSVG
						? c.createElementNS(
								"http://www.w3.org/2000/svg",
								"rect",
						  )
						: c.createElement("div");
					t.init(j),
						a.myParent.appendChild(j),
						m.each(
							["overflow", "overflowX", "overflowY"],
							(a, b) => {
								t.CSS.setPropertyValue(j, b, "hidden");
							},
						),
						t.CSS.setPropertyValue(j, "position", a.position),
						t.CSS.setPropertyValue(j, "fontSize", a.fontSize),
						t.CSS.setPropertyValue(j, "boxSizing", "content-box"),
						m.each(
							[
								"minWidth",
								"maxWidth",
								"width",
								"minHeight",
								"maxHeight",
								"height",
							],
							(a, b) => {
								t.CSS.setPropertyValue(j, b, `${h}%`);
							},
						),
						t.CSS.setPropertyValue(j, "paddingLeft", `${h}em`),
						(i.percentToPxWidth = I.lastPercentToPxWidth =
							(parseFloat(
								v.getPropertyValue(j, "width", null, !0),
							) || 1) / h),
						(i.percentToPxHeight = I.lastPercentToPxHeight =
							(parseFloat(
								v.getPropertyValue(j, "height", null, !0),
							) || 1) / h),
						(i.emToPx = I.lastEmToPx =
							(parseFloat(v.getPropertyValue(j, "paddingLeft")) ||
								1) / h),
						a.myParent.removeChild(j);
				}
				return (
					null === I.remToPx &&
						(I.remToPx =
							parseFloat(
								v.getPropertyValue(c.body, "fontSize"),
							) || 16),
					null === I.vwToPx &&
						((I.vwToPx = parseFloat(b.innerWidth) / 100),
						(I.vhToPx = parseFloat(b.innerHeight) / 100)),
					(i.remToPx = I.remToPx),
					(i.vwToPx = I.vwToPx),
					(i.vhToPx = I.vhToPx),
					t.debug >= 1 &&
						console.log(`Unit ratios: ${JSON.stringify(i)}`, f),
					i
				);
			}
			if (h.begin && 0 === y) {
				try {
					h.begin.call(o, o);
				} catch (r) {
					setTimeout(() => {
						throw r;
					}, 1);
				}
			}
			if ("scroll" === C) {
				let u;
				let w;
				let z;
				const A = /^x$/i.test(h.axis) ? "Left" : "Top";
				const D = parseFloat(h.offset) || 0;
				h.container
					? p.isWrapped(h.container) || p.isNode(h.container)
						? ((h.container = h.container[0] || h.container),
						  (u = h.container[`scroll${A}`]),
						  (z = u + m(f).position()[A.toLowerCase()] + D))
						: (h.container = null)
					: ((u =
							t.State.scrollAnchor[
								t.State[`scrollProperty${A}`]
							]),
					  (w =
							t.State.scrollAnchor[
								t.State[
									`scrollProperty${
										"Left" === A ? "Top" : "Left"
									}`
								]
							]),
					  (z = m(f).offset()[A.toLowerCase()] + D)),
					(i = {
						scroll: {
							rootPropertyValue: !1,
							startValue: u,
							currentValue: u,
							endValue: z,
							unitType: "",
							easing: h.easing,
							scrollData: {
								container: h.container,
								direction: A,
								alternateValue: w,
							},
						},
						element: f,
					}),
					t.debug &&
						console.log("tweensContainer (scroll): ", i.scroll, f);
			} else if ("reverse" === C) {
				if (!g(f).tweensContainer) {
					return void m.dequeue(f, h.queue);
				}
				"none" === g(f).opts.display && (g(f).opts.display = "auto"),
					"hidden" === g(f).opts.visibility &&
						(g(f).opts.visibility = "visible"),
					(g(f).opts.loop = !1),
					(g(f).opts.begin = null),
					(g(f).opts.complete = null),
					(s.easing || h.easing = undefined),
					(s.duration || h.duration = undefined),
					(h = m.extend({}, g(f).opts, h));
				const E = m.extend(!0, {}, g(f).tweensContainer);
				for (const F in E) {
					if ("element" !== F) {
						const G = E[F].startValue;
						(E[F].startValue = E[F].currentValue = E[F].endValue),
							(E[F].endValue = G),
							p.isEmptyObject(s) || (E[F].easing = h.easing),
							t.debug &&
								console.log(
									`reverse tweensContainer (${F}): ${JSON.stringify(
										E[F],
									)}`,
									f,
								);
					}
				}
				i = E;
			} else if ("start" === C) {
				let E;
				g(f).tweensContainer &&
					g(f).isAnimating === !0 &&
					(E = g(f).tweensContainer),
					m.each(q, (b, c) => {
						if (RegExp(`^${v.Lists.colors.join("$|^")}$`).test(b)) {
							const e = a(c, !0);
							const f = e[0];
							const g = e[1];
							const h = e[2];
							if (v.RegEx.isHex.test(f)) {
								for (
									let i = ["Red", "Green", "Blue"],
										j = v.Values.hexToRgb(f),
										k = h ? v.Values.hexToRgb(h) : d,
										l = 0;
									l < i.length;
									l++
								) {
									const m = [j[l]];
									g && m.push(g),
										k !== d && m.push(k[l]),
										(q[b + i[l]] = m);
								}
								delete q[b];
							}
						}
					});
				for (let H in q) {
					const K = a(q[H]);
					let L = K[0];
					const M = K[1];
					let N = K[2];
					H = v.Names.camelCase(H);
					const O = v.Hooks.getRoot(H);
					let P = !1;
					if (
						g(f).isSVG ||
						"tween" === O ||
						v.Names.prefixCheck(O)[1] !== !1 ||
						v.Normalizations.registered[O] !== d
					) {
						((h.display !== d &&
							null !== h.display &&
							"none" !== h.display) ||
							(h.visibility !== d &&
								"hidden" !== h.visibility)) &&
							/opacity|filter/.test(H) &&
							!N &&
							0 !== L &&
							(N = 0),
							h._cacheValues && E && E[H]
								? (N === d &&
										(N = E[H].endValue + E[H].unitType),
								  (P = g(f).rootPropertyValueCache[O]))
								: v.Hooks.registered[H]
								  ? N === d
										? ((P = v.getPropertyValue(f, O)),
										  (N = v.getPropertyValue(f, H, P)))
										: (P = v.Hooks.templates[O][1])
								  : N === d && (N = v.getPropertyValue(f, H));
						let Q;
						let R;
						let S;
						let T = !1;
						if (
							((Q = l(H, N)),
							(N = Q[0]),
							(S = Q[1]),
							(Q = l(H, L)),
							(L = Q[0].replace(
								/^([+-\/*])=/,
								(a, b) => (T = b),
								"",
							)),
							(R = Q[1]),
							(N = parseFloat(N) || 0),
							(L = parseFloat(L) || 0),
							"%" === R &&
								(/^(fontSize|lineHeight)$/.test(H)
									? ((L /= 100), (R = "em"))
									: /^scale/.test(H)
									  ? ((L /= 100), (R = ""))
									  : /(Red|Green|Blue)$/i.test(H) &&
										  ((L = (L / 100) * 255), (R = ""))),
							/[\/*]/.test(T))
						) {
							R = S;
						} else if (S !== R && 0 !== N) {
							if (0 === L) {
								R = S;
							} else {
								e = e || n();
								const U =
									/margin|padding|left|right|width|text|word|letter/i.test(
										H,
									) ||
									/X$/.test(H) ||
									"x" === H
										? "x"
										: "y";
								switch (S) {
									case "%": {
										N *=
											"x" === U
												? e.percentToPxWidth
												: e.percentToPxHeight;
										break;
									}
									case "px":
										break;
									default:
										N *= e[`${S}ToPx`];
								}
								switch (R) {
									case "%": {
										N *=
											1 /
											("x" === U
												? e.percentToPxWidth
												: e.percentToPxHeight);
										break;
									}
									case "px":
										break;
									default:
										N *= 1 / e[`${R}ToPx`];
								}
							}
						}
						switch (T) {
							case "+": {
								L = N + L;
								break;
							}
							case "-": {
								L = N - L;
								break;
							}
							case "*": {
								L *= N;
								break;
							}
							case "/":
								L = N / L;
						}
						(i[H] = {
							rootPropertyValue: P,
							startValue: N,
							currentValue: N,
							endValue: L,
							unitType: R,
							easing: M,
						}),
							t.debug &&
								console.log(
									`tweensContainer (${H}): ${JSON.stringify(
										i[H],
									)}`,
									f,
								);
					} else {
						t.debug &&
							console.log(
								`Skipping [${O}] due to a lack of browser support.`,
							);
					}
				}
				i.element = f;
			}
			i.element &&
				(v.Values.addClass(f, "velocity-animating"),
				J.push(i),
				"" === h.queue && ((g(f).tweensContainer = i), (g(f).opts = h)),
				(g(f).isAnimating = !0),
				y === x - 1
					? (t.State.calls.push([J, o, h, null, B.resolver]),
					  t.State.isTicking === !1 &&
							((t.State.isTicking = !0), k()))
					: y++);
		}
		let e;
		let h = m.extend({}, t.defaults, s);
		let i = {};
		switch (
			(g(this) === d && t.init(this),
			parseFloat(h.delay) &&
				h.queue !== !1 &&
				m.queue(this, h.queue, (a) => {
					(t.velocityQueueEntryFlag = !0),
						(g(this).delayTimer = {
							setTimeout: setTimeout(a, parseFloat(h.delay)),
							next: a,
						});
				}),
			h.duration.toString().toLowerCase())
		) {
			case "fast": {
				h.duration = 200;
				break;
			}
			case "normal": {
				h.duration = r;
				break;
			}
			case "slow": {
				h.duration = 600;
				break;
			}
			default:
				h.duration = parseFloat(h.duration) || 1;
		}
		t.mock !== !1 &&
			(t.mock === !0
				? (h.duration = h.delay = 1)
				: ((h.duration *= parseFloat(t.mock) || 1),
				  (h.delay *= parseFloat(t.mock) || 1))),
			(h.easing = j(h.easing, h.duration)),
			h.begin && !p.isFunction(h.begin) && (h.begin = null),
			h.progress && !p.isFunction(h.progress) && (h.progress = null),
			h.complete && !p.isFunction(h.complete) && (h.complete = null),
			h.display !== d &&
				null !== h.display &&
				((h.display = h.display.toString().toLowerCase()),
				"auto" === h.display &&
					(h.display = t.CSS.Values.getDisplayType(this))),
			h.visibility !== d &&
				null !== h.visibility &&
				(h.visibility = h.visibility.toString().toLowerCase()),
			(h.mobileHA =
				h.mobileHA && t.State.isMobile && !t.State.isGingerbread),
			h.queue === !1
				? h.delay
					? setTimeout(a, h.delay)
					: a()
				: m.queue(this, h.queue, (b, c) =>
						c === !0
							? (B.promise && B.resolver(o), !0)
							: ((t.velocityQueueEntryFlag = !0), void a(b)),
				  ),
			("" !== h.queue && "fx" !== h.queue) ||
				"inprogress" === m.queue(this)[0] ||
				m.dequeue(this);
	}
	let h;
	let i;
	let n;
	let o;
	let q;
	let s;
	const u =
		arguments[0] &&
		(arguments[0].p ||
			(m.isPlainObject(arguments[0].properties) &&
				!arguments[0].properties.names) ||
			p.isString(arguments[0].properties));
	if (
		(p.isWrapped(this)
			? ((h = !1), (n = 0), (o = this), (i = this))
			: ((h = !0),
			  (n = 1),
			  (o = u ? arguments[0].elements || arguments[0].e : arguments[0])),
		(o = f(o)))
	) {
		u
			? ((q = arguments[0].properties || arguments[0].p),
			  (s = arguments[0].options || arguments[0].o))
			: ((q = arguments[n]), (s = arguments[n + 1]));
		const x = o.length;
		const y = 0;
		if (!(/^(stop|finish|finishAll)$/i.test(q) || m.isPlainObject(s))) {
			const z = n + 1;
			s = {};
			for (let A = z; A < arguments.length; A++) {
				p.isArray(arguments[A]) ||
				!(
					/^(fast|normal|slow)$/i.test(arguments[A]) ||
					/^\d/.test(arguments[A])
				)
					? p.isString(arguments[A]) || p.isArray(arguments[A])
						? (s.easing = arguments[A])
						: p.isFunction(arguments[A]) &&
						  (s.complete = arguments[A])
					: (s.duration = arguments[A]);
			}
		}
		const B = { promise: null, resolver: null, rejecter: null };
		h &&
			t.Promise &&
			(B.promise = new t.Promise((a, b) => {
				(B.resolver = a), (B.rejecter = b);
			}));
		let C;
		switch (q) {
			case "scroll": {
				C = "scroll";
				break;
			}
			case "reverse": {
				C = "reverse";
				break;
			}
			case "finish":
			case "finishAll":
			case "stop": {
				m.each(o, (a, b) => {
					g(b)?.delayTimer &&
						(clearTimeout(g(b).delayTimer.setTimeout),
						g(b).delayTimer.next?.(),
						(g(b).delayTimer = undefined)),
						"finishAll" !== q ||
							(s !== !0 && !p.isString(s)) ||
							(m.each(
								m.queue(b, p.isString(s) ? s : ""),
								(a, b) => {
									p.isFunction(b) && b();
								},
							),
							m.queue(b, p.isString(s) ? s : "", []));
				});
				const D = [];
				return (
					m.each(t.State.calls, (a, b) => {
						b &&
							m.each(b[1], (c, e) => {
								const f = s === d ? "" : s;
								return f === !0 ||
									b[2].queue === f ||
									(s === d && b[2].queue === !1)
									? void m.each(o, (c, d) => {
											d === e &&
												((s === !0 || p.isString(s)) &&
													(m.each(
														m.queue(
															d,
															p.isString(s)
																? s
																: "",
														),
														(a, b) => {
															p.isFunction(b) &&
																b(null, !0);
														},
													),
													m.queue(
														d,
														p.isString(s) ? s : "",
														[],
													)),
												"stop" === q
													? (g(d)?.tweensContainer &&
															f !== !1 &&
															m.each(
																g(d)
																	.tweensContainer,
																(a, b) => {
																	b.endValue =
																		b.currentValue;
																},
															),
													  D.push(a))
													: ("finish" === q ||
															"finishAll" ===
																q) &&
													  (b[2].duration = 1));
									  })
									: !0;
							});
					}),
					"stop" === q &&
						(m.each(D, (a, b) => {
							l(b, !0);
						}),
						B.promise && B.resolver(o)),
					a()
				);
			}
			default: {
				if (!m.isPlainObject(q) || p.isEmptyObject(q)) {
					if (p.isString(q) && t.Redirects[q]) {
						const E = m.extend({}, s);
						const F = E.duration;
						const G = E.delay || 0;
						return (
							E.backwards === !0 &&
								(o = m.extend(!0, [], o).reverse()),
							m.each(o, (a, b) => {
								parseFloat(E.stagger)
									? (E.delay = G + parseFloat(E.stagger) * a)
									: p.isFunction(E.stagger) &&
									  (E.delay = G + E.stagger.call(b, a, x)),
									E.drag &&
										((E.duration =
											parseFloat(F) ||
											(/^(callout|transition)/.test(q)
												? 1e3
												: r)),
										(E.duration = Math.max(
											E.duration *
												(E.backwards
													? 1 - a / x
													: (a + 1) / x),
											0.75 * E.duration,
											200,
										))),
									t.Redirects[q].call(
										b,
										b,
										E || {},
										a,
										x,
										o,
										B.promise ? B : d,
									);
							}),
							a()
						);
					}
					const H = `Velocity: First argument (${q}) was not a property map, a known action, or a registered redirect. Aborting.`;
					return (
						B.promise ? B.rejecter(new Error(H)) : console.log(H),
						a()
					);
				}
				C = "start";
			}
		}
		const I = {
			lastParent: null,
			lastPosition: null,
			lastFontSize: null,
			lastPercentToPxWidth: null,
			lastPercentToPxHeight: null,
			lastEmToPx: null,
			remToPx: null,
			vwToPx: null,
			vhToPx: null,
		};
		const J = [];
		m.each(o, (a, b) => {
			p.isNode(b) && e.call(b);
		});
		let K;
		const E = m.extend({}, t.defaults, s);
		if (((E.loop = parseInt(E.loop)), (K = 2 * E.loop - 1), E.loop)) {
			for (let L = 0; K > L; L++) {
				const M = { delay: E.delay, progress: E.progress };
				L === K - 1 &&
					((M.display = E.display),
					(M.visibility = E.visibility),
					(M.complete = E.complete)),
					w(o, "reverse", M);
			}
		}
		return a();
	}
};
(t = m.extend(w, t)), (t.animate = w);
let x = b.requestAnimationFrame || o;
return (
				t.State.isMobile ||
					c.hidden === d ||
					c.addEventListener("visibilitychange", () => {
						c.hidden
							? ((x = (a) => setTimeout(() => {
										a(!0);
									}, 16)),
								k())
							: (x = b.requestAnimationFrame || o);
					}),
				(a.Velocity = t),
				a !== b &&
					((a.fn.velocity = w),
					(a.fn.velocity.defaults = t.defaults)),
				m.each(["Down", "Up"], (a, b) => {
					t.Redirects[`slide${b}`] = (a, c, e, f, g, h) => {
						const i = m.extend({}, c);
						const j = i.begin;
						const k = i.complete;
						const l = {
								height: "",
								marginTop: "",
								marginBottom: "",
								paddingTop: "",
								paddingBottom: "",
							};
						const n = {};
						i.display === d &&
							(i.display =
								"Down" === b
									? "inline" ===
										t.CSS.Values.getDisplayType(a)
										? "inline-block"
										: "block"
									: "none"),
							(i.begin = () => {
								j?.call(g, g);
								for (const c in l) {
									n[c] = a.style[c];
									const d = t.CSS.getPropertyValue(a, c);
									l[c] = "Down" === b ? [d, 0] : [0, d];
								}
								(n.overflow = a.style.overflow),
									(a.style.overflow = "hidden");
							}),
							(i.complete = () => {
								for (const b in n) { a.style[b] = n[b]; }
								k?.call(g, g), h?.resolver(g);
							}),
							t(a, l, i);
					};
				}),
				m.each(["In", "Out"], (a, b) => {
					t.Redirects[`fade${b}`] = function (a, c, e, f, g, h) {
						const i = m.extend({}, c);
						const j = { opacity: "In" === b ? 1 : 0 };
						const k = i.complete;
						(i.complete =
							e !== f - 1
								? (i.begin = null)
								: () => {
										k?.call(g, g), h?.resolver(g);
									}),
							i.display === d &&
								(i.display = "In" === b ? "auto" : "none"),
							t(this, j, i);
					};
				}),
				t
			);
})(window.jQuery || window.Zepto || window, window, document))
