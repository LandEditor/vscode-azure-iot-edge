...
const app = new Vue({
	...
	methods: {
		...
		importModule: async function () {
			...
			if (
				this.selectedModule.metadata.environmentVariables &&
				this.selectedModule.metadata.environmentVariables.length > 0
			) {
				environmentVariables = {};
				for (const environmentVariable of this.selectedModule.metadata
					.environmentVariables) {
			...
			let twins = undefined;
			if (
				this.selectedModule.metadata.twins &&
				this.selectedModule.metadata.twins.length > 0
			) {
				const twinObject = {};
