...
export class Simulator {
	private static iotedgehubdevVersionUrl =
	...
	private static maxRetryTimes = 3;
	private static retryInterval = 5000;
	...
	private static adjustTerminalCommand(command: string): string {
		return Simulator.currentPlatform === "linux" ||
			Simulator.currentPlatform === "darwin"
			? `sudo ${command}`
			: command;
	}
		...
		try {
			...
			if (simulatorType === SimulatorType.NotInstalled) {
				...
				const version: string | null =
					await this.getCurrentSimulatorVersion();
				if (version && semver.valid(version)) {
					const desiredVersion: string | undefined =
						await this.getDesiredSimulatorVersion(outputChannel);
			...
			const installResult =
				await this.autoInstallSimulator(outputChannel);
	...
	public async runSolution(
		...
		return await this.callWithInstallationCheck(outputChannel, async () => {
			await this.checkIoTedgehubdevConnectionString(outputChannel);
			const pattern =
				"{**/deployment.*.json,**/deployment.json,**/deployment.*.debug.json,**/config/*.json}";
			const excludePattern = `{${Constants.tsonPattern}}`;
			const deployFile: string = await Utility.getInputFilePath(
				deployFileUri,
				pattern,
	...
	private async getDesiredSimulatorInfo(outputChannel: vscode.OutputChannel) {
		if (this.desiredSimulatorInfo) {
			...
			await RetryPolicy.retry(
				Simulator.maxRetryTimes,
				Simulator.retryInterval,
				outputChannel,
				async () => {
					...
					const lockVersion =
						process.env[Simulator.iotedgehubdevLockVersionKey];
					if (
	...
	private async getCurrentSimulatorVersion(): Promise<string | undefined> {
		const output: string = await Executor.executeCMD(
			undefined,
			this.getAdjustedSimulatorExecutorPath(true),
			{ shell: true },
			"--version",
	...
	private getAdjustedSimulatorExecutorPath(forceUseCmd = false): string {
		...
		await vscode.window.withProgress(
			{
				...
				title:
					Constants.downloadingAndInstallingStandaloneSimulatorMsg +
					version,
			},
			async () => {
	...
	private async checkIoTedgehubdevConnectionString(
		...
		if (await this.isValidateConfigSupported()) {
			try {
				...
				if (
					error.errorCode === 2 &&
					Simulator.currentPlatform === "win32"
				) {
					errorMsg =
	...
	private async isSupported(supportedVersion: string): Promise<boolean> {
		...
		try {
			const output = await Executor.executeCMD(
				undefined,
				this.getAdjustedSimulatorExecutorPath(true),
				{ shell: true },
				"--version",
