...
export class ConfigDiagnosticProvider {
	...
	private async provideDeploymentTemplateDiagnostics(
		document: vscode.TextDocument,
		...
		try {
			...
			for (const moduleNode of modulesNode.children) {
				...
				const imgJsonPath: string[] =
					Constants.imgDeploymentManifestJsonPath.slice(
						Constants.moduleNameDeploymentManifestJsonPathIndex + 1,
					); // image node JSON path relative to module node
				const imageNode: parser.Node = parser.findNodeAtLocation(
					moduleNode.children[1],
					imgJsonPath,
				); // the node value is stored in its second child node
	...
	private async provideModuleManifestDiagnostics(
		document: vscode.TextDocument,
	): Promise<vscode.Diagnostic[]> {
		const diags: vscode.Diagnostic[] = [];
		...
		for (const platformNode of platformsNode.children) {
			...
			const dockerfileFullPath: string = path.join(
