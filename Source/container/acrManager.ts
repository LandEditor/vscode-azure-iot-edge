...
export class AcrManager {
	...
	private async selectAcrRegistry(): Promise<AcrRegistryQuickPickItem> {
		...
		const acrRegistryItem: AcrRegistryQuickPickItem =
			await vscode.window.showQuickPick(this.loadAcrRegistryItems(), {
	...
	private async acquireAcrRefreshToken(
		registryUrl: string,
		tenantId: string,
		...
		const acrRefreshToken = await axios
			.post(`https://${registryUrl}/oauth2/exchange`, qs.stringify(data))
			.then((res) => {
	...
	private async acquireAcrAccessToken(
		...
		const acrAccessTokenResponse = await axios.post(
			`https://${registryUrl}/oauth2/token`,
			qs.stringify({
