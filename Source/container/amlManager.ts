...
export class AmlManager {
	...
	private async selectWorkspace(): Promise<AmlWorkspaceQuickPickItem> {
		...
		const workspaceItem: AmlWorkspaceQuickPickItem =
			await vscode.window.showQuickPick(this.loadWorkspaceItems(), {
				placeHolder: `Select ${Constants.amlWorkspaceDesc}`,
				ignoreFocusOut: true,
			});
	...
	private async loadAmlImageItems(
		...
		try {
			...
			if (
				result.parsedBody.value === undefined ||
				result.parsedBody.value.length === 0
			) {
